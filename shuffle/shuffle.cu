...
int main(){
    ...
    cudaMalloc((int**)&d_inData, nBytes);
    cudaMalloc((int**)&d_outData, nBytes);
    ...
    // shfl bcast
    test_shfl_broadcast<<<1, block>>>(d_outData, d_inData, 18);
    cudaGetLastError();
    ...
    // shfl up
    test_shfl_up<<<1, block>>>(d_outData, d_inData, 4);
    cudaGetLastError();
    cudaMemcpy(h_outData, d_outData, nBytes, cudaMemcpyDeviceToHost);
    ...
    // shfl xor
    test_shfl_xor<<<1, block>>>(d_outData, d_inData, 2);
    cudaGetLastError();
    cudaMemcpy(h_outData, d_outData, nBytes, cudaMemcpyDeviceToHost);
